...
@section('title', 'Etkinlik ve Kamp Fotoğrafları — BRY Kampı ve Özel Buluşmalar')
@section('description', 'Bilinçli Ritmik Yaşam Kampı, özel buluşmalar ve atölyelerden kareler. Gerçek anların ve birlikte yaşanan deneyimlerin izleri.')
...
@section('canonical', 'https://www.bilincliritmikyasam.com/etkinlik-ve-kamp')
...
@php
  $gallery = \App\Models\GalleryPhoto::active()->orderBy('sort')->orderBy('id')->get();
@endphp

@section('content')
<!-- PAGE HERO -->
  <section class="page-hero" aria-labelledby="etkinlik-title">
    <div class="container">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <a href="#">Deneyimler</a>
        <span class="sep" aria-hidden="true">/</span>
        <span aria-current="page">Etkinlik ve Kamp Fotoğrafları</span>
      </nav>
      <span class="eyebrow">Deneyimler</span>
      <h1 id="etkinlik-title">Etkinlik ve Kamp <em>Fotoğrafları</em></h1>
      <p class="lead">Bilinçli Ritmik Yaşam Kampı ve özel buluşmalarımızdan kareler…</p>
      <p class="lead">Bu fotoğraflar, birlikte geçirilen anların ve gerçek deneyimlerin izlerini taşır.</p>
    </div>
  </section>

  <!-- GALLERY -->
  <section aria-labelledby="galeri-title" style="padding-top: 30px; padding-bottom: 90px;">
    <div class="container">
      <h2 id="galeri-title" class="visually-hidden" style="position:absolute;left:-9999px;">Fotoğraf galerisi</h2>
      ...
      @else
        <div class="photo-grid" id="photo-grid">
          @foreach($gallery as $i => $p)
            <a href="{{ $p->imageUrl() }}"
               class="photo-cell"
               data-lightbox="etkinlik"
               data-index="{{ $i }}"
               aria-label="{{ $p->caption ?: 'Fotoğraf #' . ($i + 1) }}">
              <img src="{{ $p->imageUrl() }}" alt="{{ $p->caption ?: 'Etkinlik fotoğrafı ' . ($i + 1) }}" loading="lazy">
              @if($p->caption)
