...
@section('title', 'Etkinlik & Kamp Fotoğrafları')

@section('content')
<div class="adm-head">
  <div>
    <h1>Etkinlik & Kamp Fotoğrafları</h1>
    <div class="meta">{{ $items->count() }} fotoğraf · sürükle-bırak ile sıralayabilirsin</div>
  </div>
  <a class="adm-btn" href="{{ route('admin.gallery.create') }}">+ Yeni Fotoğraf</a>
</div>
...
@if($items->isEmpty())
  <div class="adm-card" style="text-align:center; padding: 40px;">
    ...
    <a class="adm-btn" href="{{ route('admin.gallery.create') }}">İlk Fotoğrafı Ekle</a>
    <p style="margin-top: 16px; font-size: 13px; color: var(--a-ink-mute);">İpucu: <code>php artisan db:seed --class=GalleryPhotoSeeder</code> ile <code>storage/app/public/etkinlik-kamp/</code> klasöründeki mevcut fotoğrafları toplu içe aktarabilirsin.</p>
  </div>
@else
  <div id="gallery-grid"
       class="gph-grid"
    ...
    @foreach($items as $p)
      <div class="gph-card" data-id="{{ $p->id }}">
        ...
        <div class="gph-actions">
          <a class="adm-btn adm-btn--ghost adm-btn--sm" href="{{ route('admin.gallery.edit', $p) }}">Düzenle</a>
          <form action="{{ route('admin.gallery.destroy', $p) }}" method="POST" style="display:inline;" onsubmit="return confirm('Silmek istediğine emin misin?');">
            @csrf @method('DELETE')
