...
<div class="adm-head">
  ...
  <a class="adm-btn adm-btn--ghost" href="{{ route('admin.gallery.index') }}">← Geri</a>
</div>
...
<form action="{{ $item->exists ? route('admin.gallery.update', $item) : route('admin.gallery.store') }}" method="POST" enctype="multipart/form-data">
  ...
  <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px; align-items: start;">
    <div>
      <div class="adm-card">
        <h2>Görsel</h2>
        <p class="sub">JPG / PNG / WEBP · max 8 MB. Etkinlik kareleri 4:3 veya yatay önerilir.</p>
        ...
        <div class="adm-field">
          ...
          <input id="caption" type="text" name="caption" value="{{ old('caption', $item->caption) }}" placeholder="BRY Kampı · Açılış">
        </div>
      </div>
    </div>
  ...
  <div style="display: flex; gap: 10px;">
    ...
    <a class="adm-btn adm-btn--ghost" href="{{ route('admin.gallery.index') }}">İptal</a>
  </div>
</form>
@endsection
